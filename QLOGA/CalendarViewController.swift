...
struct CalendarPickerView: View {
    @State var bottomSheetPosition: NewBottomSheetPosition = .bottom //1
    ...
    var body: some View {
        CalendarPicker()
            ...
            .bottomSheet(bottomSheetPosition: $bottomSheetPosition, options: [.allowContentDrag, .swipeToDismiss, .tapToDissmiss, .backgroundBlur(effect: .extraLight), .cornerRadius(25), .shadow(color: .lightGray, radius: 3, x: 3, y: 3)], headerContent: {
                HStack(alignment: .center) {
                    Spacer()
                    Text("Day's workload")
                        .foregroundColor(Color.secondary.opacity(0.8)).multilineTextAlignment(.center).font(Font.system(size: 20, weight: .medium, design: .default))
                    Spacer()
                }
            }, mainContent: {
                HStack(alignment: .top) {
                    VStack(alignment: .center, spacing: 10) {
                        Circle().frame(width: 16, height: 16, alignment: .top).foregroundColor(.Pink).shadow(color: .Pink, radius: 1, x: 0, y: 0)
                        Text("working hours").foregroundColor( .lightGray).font(.system(size: 16, weight: .regular, design: .rounded)).multilineTextAlignment(.center)
                    }
                    Spacer()
                    VStack(alignment: .center, spacing: 10) {
                        Circle().frame(width: 16, height: 16, alignment: .top).foregroundColor(Color.init("#D7FFEB")).shadow(color: Color.init("#D7FFEB"), radius: 1, x: 0, y: 0)
                        Text("current inquiry").foregroundColor( .lightGray).font(.system(size: 16, weight: .regular, design: .rounded)).multilineTextAlignment(.center)
                    }
                    Spacer()
                    VStack(alignment: .center, spacing: 10) {
                        Circle().frame(width: 16, height: 16, alignment: .top).foregroundColor(Color.init("#FFDDB6")).shadow(color: Color.init("#FFDDB6"), radius: 1, x: 0, y: 0)
                        Text("other orders").foregroundColor( .lightGray).font(.system(size: 16, weight: .regular, design: .rounded)).multilineTextAlignment(.center)
                    }
                }.padding([.horizontal,.top],20).frame(height: 160, alignment: .top)
            })
            .toolbar {
                Button {
                    if bottomSheetPosition == .bottom { bottomSheetPosition = .middle } else { bottomSheetPosition = .bottom }
                } label: {
...
//        dayViewController.dayView.timelineScrollOffset = CGPoint(x: -5, y: 5)
//        dayViewController.dayView.timelinePagerView.timelineScrollOffset = 20
...
final class CalendarViewController: DayViewController, EKEventEditViewDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        title = "Calendar"
//        self.dayView.backgroundColor = .cyan
