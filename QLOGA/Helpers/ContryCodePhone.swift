...
class ObservableCountryCode: ObservableObject {
    var countryDictionary = ["AF": "93", "AL": "355", "DZ": "213", "US": "1",
...
struct CountryCodes: View {
    ...
    var body: some View {
        GeometryReader { geo in
            VStack {
                List(self.$codeContryCode.countryDictionary.wrappedValue.sorted(by: <).filter { $0.value.hasPrefix($countryCode.wrappedValue) }, id: \.key) { key, value in
                    HStack {
                        Text("\(self.codeContryCode.flag(country: key))")
                        Text("\(self.codeContryCode.countryName(countryCode: key) ?? key)")
                        Spacer()
                        Text("+\(value)").foregroundColor(.secondary)
                    }.background(Color.white)
                        .font(.system(size: 20))
                        .onTapGesture {
                            self.$countryName.wrappedValue = self.codeContryCode.countryName(countryCode: key) ?? key
                            self.$countryCode.wrappedValue = value
                            self.$countryFlag.wrappedValue = self.codeContryCode.flag(country: key)
                            self.$y.wrappedValue = false
                        }
                }
            }
