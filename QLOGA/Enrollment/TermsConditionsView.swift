...
struct TermsConditionsView: View {
    ...
    var body: some View {
        VStack(alignment: .center, spacing: 10) {
            ...
            HStack {
               ScrollView {
                   Markdown(termsText)// Text(termsText)
            ...
            NavigationLink( destination: EnrolledProfileView(actorType: actorType)) {
                HStack {
                    Text("Accept")
                        .lineLimit(1)
                        .ignoresSafeArea(.all)
                        .shadow(color: termsAccepted ? Color.secondary : Color.white, radius: 1, x: 1, y: 1)
                        .font(.system(size: 20, weight: .regular, design: .rounded))
                        .foregroundColor(termsAccepted ? Color.white : Color.lightGray)
                        .frame(width: UIScreen.main.bounds.width - 40, height: 50)
                        .background(RoundedRectangle(cornerRadius: 25)
                            .fill(termsAccepted ? Color("AccentColor") : Color.white))
                        .clipShape(RoundedRectangle(cornerRadius: 25))
                        .shadow(color: termsAccepted ? Color.lightGray : Color.white, radius: 4, x: -4.5, y: -4.5)
                        .clipShape(RoundedRectangle(cornerRadius: 25))
                        .overlay(RoundedRectangle(cornerRadius: 25)
                            .stroke(lineWidth: 2.0)
                            .foregroundColor(termsAccepted ? Color.white : Color.lightGray)
                            .shadow(color: termsAccepted ?  .secondary.opacity(0.5) : .white, radius: 3, y: 3))
                }
