...
//			Divider().background(Color.lightGray).padding(.horizontal, -50)
		...
		}.padding(.horizontal, 20).overlay(ImageViewer(image: $image,
		...
		VStack(alignment: .center, spacing: 0) {
			Label {
				Text("Cleaning")
					...
					.font(Font.system(size: 17, weight: .regular, design: .rounded))
				Spacer()
                Text("\($provider.wrappedValue.choicedServices[ServiceType.Cleaning.id].services.count.description)")
					.foregroundColor(Color.secondary)
					.font(Font.system(size: 17, weight: .regular, design: .rounded))
			} icon: {
				Image("Cleaning")
			...
			ScrollView {
				ForEach(Array($provider.choicedServices[ServiceType.Cleaning.id].services.wrappedValue), id: \.self) { service in
...
struct ProfilePublicView_Previews: PreviewProvider {
	static var previews: some View {
		NavigationView {
			ProfilePublicView(actorType: .CUSTOMER, customer: .constant(testCustomer), provider: .constant(testProvider))
		}.previewDevice("iPhone 6s").previewInterfaceOrientation(.portraitUpsideDown)
	}
}
