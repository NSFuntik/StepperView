	...
	var body: some View {
		VStack(alignment: .center, spacing: 5) {
			...
			ScrollView(showsIndicators: false) {
				if !$provider.choicedCleaningServices.wrappedValue.isEmpty {
					VStack {
						HStack {
							Text("CHOICED SERVICES")
				...
				VStack {
					ForEach(Services[selectedButton].types, id: \.self) { type in
						Section {
							NavigationLink {
								ProviderAddServicesView(choisedServices: $provider.choicedCleaningServices,
								...
								HStack {
									Text(type.title)
										...
										.padding(.leading, 10)
									Spacer()
									Text($provider.choicedCleaningServices.wrappedValue.contains(ServiceCleaningType(rawValue: type.id) ?? .CompleteHome) ? "£160.00" : "")
		...
		ScrollView(.horizontal, showsIndicators: false) {
			HStack {
				ForEach(Services) { service in
					Button { selectedButton = service.id }
                    label: {
							VStack {
								Image(service.image)
									.resizable()
